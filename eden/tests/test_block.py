...
class TestBlock(unittest.TestCase):
    ...
    def test_block_build_pydantic_model(self):
        ...
        my_args = {
                ...
                'input_image': Image('images/cloud.jpg')          ## images require eden.datatypes.Image()
            }
        ...
        self.assertTrue(pydantic_model.prompt == my_args['prompt'], f"expected {pydantic_model.prompt} to be equal to {my_args['prompt']}")
        self.assertTrue(pydantic_model.number == my_args['number'], f"expected {pydantic_model.number} to be equal to {my_args['number']}")

        ## add more data types as they come
        self.assertTrue(pydantic_model.input_image == my_args['input_image'], f"expected {pydantic_model.input_image} to be equal to {my_args['input_image']}")
