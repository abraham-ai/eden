...
parser.add_argument('-ll','--log-level', help='log level', required=False, type= str, default= 'info')
parser.add_argument('-rg','--requires-gpu', help='set to True if you want to check if eden is on a GPU runtime', required=False, default= False, type= bool)
parser.add_argument('-mnw','--max-num-workers', help='maximum number of workers to be run in parallel', required=False, default= 1, type= int)
parser.add_argument('-ho','--host', help= 'host name', required=False, default= '0.0.0.0', type= str)

## migh also need '172.17.0.1'
parser.add_argument('-rh','--redis-host', help= 'redis host name', required=False, default= 'localhost', type= str)
